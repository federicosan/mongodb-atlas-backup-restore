...
export default class MongoBackup {
    constructor(config = configuration) {
        return new Promise((resolve, reject) => {
            this.extendArguments(config.mongo_atlas_configuration)
                .then((res) => {
                    this.atlasArgs = res;
                    if (res)
                        info('atlasArgs set');
                    return this.extendArguments(config.restore_db_configuration, false);
                })
                .then((res) => {
    ...
    checkExistingDatabase(config = configuration) {
        return new Promise((resolve, reject) => {
            ...
            let usrpwd = !config.restore_db_configuration.user ? undefined : !config.restore_db_configuration.password ? "" : `${config.restore_db_configuration.user}:${config.restore_db_configuration.password}@`;
            let connection = "";
            if (config.restore_db_configuration.nodes) {
                connection += `mongodb://${config.restore_db_configuration.user}:${config.restore_db_configuration.password}@${config.restore_db_configuration.nodes.join(`,`)}`;
                connection += `/${config.restore_db_configuration.database}?replicaSet=${config.restore_db_configuration.replicaSet}`;
            ...
            if (config.restore_db_configuration.host) {
                connection += `mongodb://${!usrpwd ? "" : usrpwd }${config.restore_db_configuration.host}:${config.restore_db_configuration.port || 27017}`;
            }
            ...
            mongoose.connect(connection, options, (err) => {
                if (err) {
                    ...
                    //Todo Check if the DB exists and delete it
                    //Todo Before delete ask for Prompt - Sure, (Y/n)
